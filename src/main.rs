...
fn draw_meme(board: &GameBoard, blk: &Block, meme: Meme) {
    if meme == NO_MEME {
        return;
    }

    let region = board.calc_block_region(blk.column, blk.row);
...
fn draw_game_board(board: &GameBoard, game_map: &GameMap) {
    for i in 0..game_map.columns {
        for j in 0..game_map.rows {
            let blk = Block { column: i, row: j };
...
fn draw_connector(board: &GameBoard, trace: &[Block]) {
    for triplet in trace.windows(2) {
        let first_region = board.calc_block_region(triplet[0].column, triplet[0].row);
        ...
        let second_region = board.calc_block_region(triplet[1].column, triplet[1].row);
...
async fn main() {
    ...
    let mut interest_blocks = empty_blocks
        .iter()
        .filter(|blk| !(4..7).contains(&blk.column))
        .filter(|blk| !(4..7).contains(&blk.row))
        .copied()
        .collect::<Vec<&Block>>();
    ...
    game_map
        ...
        .unwrap();
    game_map.set_meme(10, &Block { column: 4, row: 4 }).unwrap();
    game_map.set_meme(10, &Block { column: 6, row: 6 }).unwrap();
    ...
    loop {
        ...
        let board = GameBoard::new(screen, game_map.columns, game_map.rows);
        draw_game_board(&board, &game_map);

        if is_mouse_button_down(MouseButton::Left) {
            let trace = game_map
                .connect(&Block { column: 4, row: 4 }, &Block { column: 6, row: 6 })
                .unwrap();
